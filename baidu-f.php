...
if (strlen($s) > 0) {
    ...
    if (strlen($cache->get(urlencode($querys))) > 0 && ($wcache + $wz) < 48) {
        ...
        $options = array (
            ...
            CURLOPT_POSTFIELDS => 'http:'.$url.$l.$querys,
...
echo '</title>
<meta content="';
if (file_exists($stk.urlencode(@$query).'-abs.txt')) {
    echo file_get_contents($stk.urlencode($query).'-abs.txt');
}
else {
...
if (strlen(@$mnum[0]) > 0) {
    ...
    <p class="white center">
        '.$mnum[0].'&nbsp;个结果
        <a class="noa" href="//open.baidu.com/special/time/" target="_blank" rel="external nofollow noreferrer" title="现在几点？">'.date('Y-m-d H:i:s', time()).'</a>
    </p>';
            ...
            if (preg_match_all("/(?<=F1':)(\s?)(')([0-9A-F]{1})([0-9A-F]{1})([0-9A-F]{1})([0-9A-F]{1})([0-9A-F]{1})([0-9A-F]{1})([0-9A-F]{1})([0-9A-F]{1})(?=',)/", $se, $mf1)) {
                ...
                if (preg_match_all("/(?<=F2':)(\s?)(')([0-9A-F]{1})([0-9A-F]{1})([0-9A-F]{1})([0-9A-F]{1})([0-9A-F]{1})([0-9A-F]{1})([0-9A-F]{1})([0-9A-F]{1})(?=',)/", $se, $mf2)) {
                    foreach ($msrcid[3] as $i => $v) {
                        ...
                        // 相关检索词
                        elseif ($mf1[8][$i] == '5') {
                            $sug9[$i] = '&nbsp;<a class="pinks" href="//www.weixingon.com/baidusp-hot.php?s='.$query.'" title="F1&nbsp;=&nbsp;xxxxx5xx&nbsp;最新其他人还搜索的相关热门内容" target="_blank" rel="external nofollow noreferrer">新热门内容</a>';
                        }
                        elseif ($mf1[8][$i] == '3') {
                            $sug9[$i] = '&nbsp;<a class="pinks" href="//www.weixingon.com/baidusp-hot.php?s='.$query.'" title="F1&nbsp;=&nbsp;xxxxx5xx&nbsp;中期其他人还搜索的相关热门内容" target="_blank" rel="external nofollow noreferrer">中热门内容</a>';
                        }
                        elseif ($mf1[8][$i] == '0') {
...
array(29094, '找课程&nbsp;百度教育', '', '', 'ec'),
array(29093, '机构&nbsp;百度教育', '', '', 'ec'),
array(29090, '课程&nbsp;百度教育', '', '', 'ec'),
array(29089, $queryn.'&nbsp;百度健康', '', '', 'ec'),
...
array(1521, '图片&nbsp;百度百科(与查询词内容相关度较高)', '', '', 'ec'),
...
array(43, '去百度知道提问', '', '', 'ec'),
array(37, $queryn.'&nbsp;最新图像', '', '', 'ec'),
array(34, $queryn.'&nbsp;最新微博结果', '', '', 'ec'),
array(23, $queryn.'&nbsp;百度翻译', '', '', 'ec'),
array(19, $queryn.'&nbsp;最新相关消息', '', '', 'ec'),
array(10, '百度贴吧', '', '', 'sp'),
array(5, $queryn.'&nbsp;百度音乐', '', '', 'ec'),
array(4, $queryn.'&nbsp;百度图片', '', '', 'ec'),
...
'//fanyi.baidu.com/#auto/zh/'.$query,
        ...
        foreach ($n as $i => $v) {
            foreach ($srcid as $h => $v) {
                ...
                <tr class="back-egg">
                    ...
                    <td class="center">
                        <a href="//www.weixingon.com/baidusp-srcid.php" target="_blank" rel="external nofollow noreferrer">'.$srcid[$h][4].'</a>
                    </td>
                    <td class="center">
                        <a target="_blank" href="//www.weixingon.com/baidusp-op.php?srcid='.$n[$i][0].'&amp;s='.preg_replace('/(\s+)/', '+', $n[$i][1]).'" rel="external nofollow noreferrer" title="模板&nbsp;'.$n[$i][4].'">'.$n[$i][0].'</a>
                    </td>
                </tr>';
                    unset($n[$i]);
                }
                elseif (@$n[$i][0] == $srcid[$h][0] && $srcid[$h][4] == 'ec') {
                    if ($srcid[$h][0] == 1 || $srcid[$h][0] == 4 || $srcid[$h][0] == 19 || $srcid[$h][0] == 23 || $srcid[$h][0] == 34 || $srcid[$h][0] == 101 || $srcid[$h][0] == 1521 || $srcid[$h][0] == 1527 || $srcid[$h][0] == 1532 || $srcid[$h][0] == 1537 || $srcid[$h][0] == 1542 || $srcid[$h][0] == 1547 || $srcid[$h][0] == 4004) {
                        echo '
                <tr class="back-egg">
                    <td>
                        '.$n[$i][2].'&nbsp;<a target="_blank" href="'.preg_replace($np, $nr, $n[$i][3]).'" rel="external nofollow noreferrer">'.$srcid[$h][1].'</a>
                    </td>
                    <td class="center">
                        sp';
                    }
                ...
                <tr class="back-orange">
                    ...
                    <td class="center">
                        <a href="//ask.seowhy.com/question/9186" target="_blank" rel="external nofollow noreferrer" title="为什么有的搜索结果会没有百度参数，这样的现象原因是什么">'.$srcid[$h][4].'</a>';
                    }
                    echo '
                    </td>
    ...
    <div class="draglist" draggable="true">
        ...
        if (count(@$mrelated[2]) > 0 && (count(@$sug1[1]) + count(@$sug2[1]) + count(@$sugunion)) == 0) {
            ...
            foreach (@$mrelated[2] as $i => $v) {
                ...
                echo '
                    <td><a href="'.$url.$l.preg_replace('/(\s+)/', '%20', @$mrelated[2][$i]).'" target="_blank">'.@$mrelated[2][$i].'</a></td>';
                $i++;
                if ($i % 3 == 0) {
                    echo '
                </tr>';
            ...
            if (count($mrelated[2]) % 3 == 1)  {
                echo '
                    <td><a href="//www.zhihu.com/question/29778227/answer/45587654" target="_blank" rel="external nofollow noreferrer">能百度出多少页</a></td>
                    <td></td>
                </tr>';
            }
            ...
            echo '
                    <th>相关搜索</th>
                    <th>下拉提示模式&nbsp;I</th>
                    <th><a href="//ask.seowhy.com/article/109" rel="external nofollow noreferrer" target="_blank" title="百度相关提示与搜索结果标题">下拉提示模式&nbsp;II</a></th>
                    <th>搜索框提示</th>
                </tr>
            </thead>
            <tbody>';
            ...
            for ($i = 0; $i < (max(count(@$mrelated[2]), count(@$sug1[1]), count(@$sug2[1]), count(@$sugunion)) - 1); $i++) {
                ...
                if (strlen(@$mrelated[2][$i]) > 0) {
                    echo '<a href="'.$url.$l.preg_replace('/(\s+)/', '%20', @$mrelated[2][$i]).'" target="_blank">'.@$mrelated[2][$i].'</a>';
                }
                if (count(@$mrelated[2]) == 9 && $i == 9) {
                    echo '<a href="//www.zhihu.com/question/29778227/answer/45587654" target="_blank" rel="external nofollow noreferrer">能百度出多少页</a>';
                }
                echo '</td>
                    <td>';
                if (strlen(@$sug1[1][$i]) > 0) {
                    echo '<a href="'.$url.$l.preg_replace('/(\s+)/', '+', @$sug1[1][$i]).'" target="_blank">'.@$sug1[1][$i].'</a>';
                }
                echo '</td>
                    <td>';
                if (strlen(@$sug2[1][$i]) > 0) {
                    echo '<a href="'.$url.$l.preg_replace('/(\s+)/', '+', @$sug2[1][$i]).'" target="_blank">'.@$sug2[1][$i].'</a>';
                }
                echo '</td>
                    <td>';
                if (strlen(@$sugunion[$i+1]) > 0) {
                    echo '<a href="'.$url.$l.preg_replace('/(\s+)/', '+', @$sugunion[$i+1]).'" target="_blank">'.@$sugunion[$i+1].'</a>';
                }
                echo '</td>
                </tr>';
            }
    ...
    <table>
        <thead>
            <tr>
                <th><a href="//ask.seowhy.com/article/53" target="_blank" rel="external nofollow noreferrer" title="百度搜索结果页参数y - 验证码与工具">摘要&nbsp;abstract</a></th>
            </tr>
        </thead>
        <tbody class="break">';
    ...
    if (strlen(@$mabs[0][0]) > 0 && file_exists($stk.urlencode(@$query).'-abs.txt') == false) {
        file_put_contents($stk.urlencode($query).'-abs.txt', strip_tags($mabs[0][0]), LOCK_EX);
    }
}
...
if (strlen($s) > 0) {
    ...
    <table>
        ...
        foreach ($maround[0] as $i => $v) {
            ...
            echo '
                <td>
                    <a href="'.$url.$l.preg_replace('/(\s+)/', '%20', $maround[0][$i]).'" target="_blank">'.$maround[0][$i].'</a>
                </td>';
            $i++;
            if ($i % 4 == 0) {
                echo '
        ...
        if (count($maround[0]) % 4 == 1)  {
            echo '
                ...
                <td>
                    <a href="//www.weixingon.com/feed.xml" target="_blank" rel="nofollow noreferrer">feed&nbsp;订阅更新日志</a>
                </td>
                <td>
